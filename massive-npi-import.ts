...
// Massive import with comprehensive search terms and geographic coverage
async function massiveNpiImport() {
  console.log('🚀 Starting massive NPI import for comprehensive speech therapy coverage...');
  
  // Expanded US cities with coordinates for better coverage
  const cityCoords: { [key: string]: { lat: number, lon: number, state: string } } = {
    ...
    'SPRINGFIELD': { lat: 37.1883, lon: -93.2923, state: 'MO' },
    'PASADENA': { lat: 34.1478, lon: -118.1445, state: 'CA' },
    'FORT COLLINS': { lat: 40.5853, lon: -105.0844, state: 'CO' },
    'HAYWARD': { lat: 37.6688, lon: -122.0808, state: 'CA' },
    'POMONA': { lat: 34.0522, lon: -117.7494, state: 'CA' },
    'CARY': { lat: 35.7915, lon: -78.7811, state: 'NC' },
    'ROCKVILLE': { lat: 39.0840, lon: -77.1528, state: 'MD' },
    'ALEXANDRIA': { lat: 38.8048, lon: -77.0469, state: 'VA' },
    'ESCONDIDO': { lat: 33.1192, lon: -117.0864, state: 'CA' },
    'MCKINNEY': { lat: 33.1973, lon: -96.6154, state: 'TX' },
  ...
  // Comprehensive search terms for speech therapy providers
  const searchTerms = [
    'speech therapy',
    'speech pathologist',
  ...
  for (const term of searchTerms) {
    ...
    const url = `https://clinicaltables.nlm.nih.gov/api/npi_org/v3/search?terms=${encodedTerm}&maxList=800&ef=name.full,addr_practice.city,addr_practice.state,addr_practice.line1,addr_practice.phone`;
    ...
    try {
      ...
      for (let i = 0; i < Math.min(npis.length, 300); i++) { // Process more per term
        const npi = npis[i];
        ...
        // Randomize cost level and services for variety
        ...
        const teletherapy = Math.random() > 0.7; // 30% offer teletherapy
    ...
    // Small delay between search terms to be respectful to the API
...
massiveNpiImport().catch(console.error);